...
open class FPNCountryRepository {
    ...
    private func getAllCountries() -> [FPNCountry] {
        ...
        do {
            if let jsonObjects = try JSONSerialization.jsonObject(with: jsonData!, options: .allowFragments) as? NSArray {
    ...
    private func getAllCountries(excluding countryCodes: [FPNCountryCode]) -> [FPNCountry] {
        ...
        for countryCode in countryCodes {
            allCountries.removeAll(where: { $0.code == countryCode })
        }
        return allCountries
    }
    ...
    private func getAllCountries(equalTo countryCodes: [FPNCountryCode]) -> [FPNCountry] {
        ...
        for countryCode in countryCodes {
            for country in allCountries where country.code == countryCode {
                countries.append(country)
            }
        }
        return countries
