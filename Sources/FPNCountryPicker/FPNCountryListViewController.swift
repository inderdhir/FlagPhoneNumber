...
open class FPNCountryListViewController: UITableViewController, UISearchResultsUpdating, UISearchControllerDelegate {
    ...
    open var showCountryPhoneCode = true
    open var searchController = UISearchController(searchResultsController: nil)
    ...
    override open func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        if searchController.isActive, let count = searchController.searchBar.text?.count, count > 0 {
    ...
    open func updateSearchResults(for searchController: UISearchController) {
        ...
        if countries.isEmpty {
            results?.removeAll()
            return
        } else if searchController.searchBar.text?.isEmpty ?? false {
            results?.removeAll()
